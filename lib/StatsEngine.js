...
StatsEngine.prototype.getNumberOfTopicLabelAnnotations = function(mode, fnCallback){
    var db = this.db;
    if(mode === "term_mode" || mode === "label_mode"){
        db.query('SELECT COUNT(DISTINCT userusername, documentid, timestamp) AS count FROM topiclabelannotation WHERE mode=?', [mode], function(err, rows, fields) {
            if (!err){
                if(rows){
                    fnCallback(rows[0]);
                }
...
StatsEngine.prototype.calculateAccuracy = function(mode, fnCallback){
    var db = this.db;
    if(mode === "term_mode" || mode === "label_mode"){
        db.query('SELECT AVG(accuracy) AS a FROM (SELECT userusername, timestamp, tps, tns, fps, fns, ((tps+tns)/(tps+tns+fps+fns)) AS accuracy '
                +'FROM (SELECT userusername, timestamp, SUM(tp) AS tps, SUM(tn) AS tns, SUM(fp) AS fps, SUM(fn) AS fns FROM (SELECT userusername, '
                +'timestamp, entitytitle, isoriginal, isselected, (isoriginal AND isselected) AS tp, (NOT isoriginal AND NOT isselected) AS tn, '
                +'(NOT isoriginal AND isselected) AS fp, (isoriginal AND NOT isselected) AS fn FROM (SELECT * FROM topiclabelannotation WHERE '
                +'(userusername, timestamp) IN (SELECT userusername, timestamp FROM (SELECT isselected, timestamp, userusername FROM topiclabelannotation '
                +'WHERE mode=? GROUP BY isselected, timestamp, userusername HAVING COUNT(*) <3) AS a) AND (userusername, timestamp) IN '
                +'(SELECT userusername, timestamp FROM (SELECT * FROM topiclabelannotation WHERE (userusername, timestamp) IN '
                +'(SELECT userusername, timestamp FROM (SELECT isselected, timestamp, userusername FROM topiclabelannotation WHERE mode=? '
                +'GROUP BY isselected, timestamp, userusername HAVING COUNT(*) <3) AS a) GROUP BY entitytitle, userusername HAVING isoriginal=1 '
                +'AND COUNT(*) =1) AS b) UNION SELECT * FROM topiclabelannotation WHERE (userusername, timestamp) IN (SELECT userusername, timestamp FROM '
                +'(SELECT isselected, timestamp, userusername FROM topiclabelannotation WHERE mode=? GROUP BY isselected, timestamp, userusername '
                +'HAVING COUNT(*) <3) AS a) AND (userusername, timestamp) IN (SELECT userusername, timestamp FROM (SELECT userusername, entitytitle, timestamp, '
                +'MIN(timestamp) AS earliest, isoriginal FROM topiclabelannotation WHERE (userusername, timestamp, isselected) IN '
                +'(SELECT isselected, timestamp, userusername FROM topiclabelannotation WHERE mode=? GROUP BY isselected, timestamp, userusername HAVING COUNT(*) <3) '
                +'GROUP BY entitytitle, userusername HAVING isoriginal=1 AND COUNT(*) >1) AS c)) AS d) AS e GROUP BY userusername, timestamp) AS f) AS g', [mode, mode, mode, mode], function(err, rows, fields) {
            if (!err){
                if(rows){
                    fnCallback(rows[0]);
                }
