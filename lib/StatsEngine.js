...
StatsEngine.prototype.getNumberOfTopicLabelAnnotationsNothingSelected = function(fnCallback){
    var db = this.db;
    db.query('SELECT COUNT(*) AS count FROM (SELECT * FROM topiclabelannotation GROUP BY UserUsername, timestamp, documentid, isselected HAVING COUNT(*) = 3) AS t', function(err, rows, fields) {
        if (!err){
            if(rows){
                fnCallback(rows[0]);
            }
