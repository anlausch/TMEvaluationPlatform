...
StatsEngine.prototype.getNumberOfTopicLabelAnnotationsNothingSelected = function(mode, fnCallback){
    var db = this.db;
    if(mode === "term_mode" || mode === "label_mode"){
        db.query('SELECT COUNT(*) AS count FROM (SELECT userusername, timestamp, documentid, isselected FROM topiclabelannotation WHERE mode=? GROUP BY userusername, timestamp, documentid, isselected HAVING COUNT(*) = 3) AS t', [mode], function(err, rows, fields) {
            if (!err){
                if(rows){
                    fnCallback(rows[0]);
                }
        ...
        db.query('SET SQL_MODE=""', function(err, rows, fields) {
            if (!err){
                db.query('SELECT AVG(accuracy) AS a FROM (SELECT userusername, timestamp, tps, tns, fps, fns, ((tps+tns)/(tps+tns+fps+fns)) AS accuracy '
                        +'FROM (SELECT userusername, timestamp, SUM(tp) AS tps, SUM(tn) AS tns, SUM(fp) AS fps, SUM(fn) AS fns FROM '
                        +'(SELECT userusername, timestamp, entitytitle, isoriginal, isselected, (isoriginal AND isselected) AS tp, (NOT isoriginal AND NOT isselected) AS tn, '
                        +'(NOT isoriginal AND isselected) AS fp, (isoriginal AND NOT isselected) AS fn FROM (SELECT * FROM topiclabelannotation WHERE (userusername, timestamp) IN '
                        +'( SELECT userusername, timestamp FROM (SELECT * FROM topiclabelannotation WHERE mode=? GROUP BY entitytitle, userusername HAVING isoriginal=1 AND COUNT(*) =1) AS a) '
                        +'UNION SELECT * FROM topiclabelannotation WHERE (userusername, timestamp) IN '
                        +'( SELECT userusername, MIN(timestamp) AS timestamp FROM (SELECT * FROM topiclabelannotation WHERE mode=? '
                        +'GROUP BY entitytitle, userusername HAVING isoriginal=1 AND COUNT(*) >1) AS b)) AS c) AS d GROUP BY userusername, timestamp) AS e) AS f', [mode, mode], function(err, rows, fields) {
                    if (!err){
                        if(rows){
                            fnCallback(rows[0]);
                        }
