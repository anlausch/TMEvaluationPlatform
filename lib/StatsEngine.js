...
StatsEngine.prototype.calculateMAP = function(fncallback){
    var db = this.db;
    var self = this;
    db.query('Select distinct email_id from annotation;', function(err, rows, fields) {
        if (!err){
          if(rows){
              var no_docs = rows.length;
              var avg_precisions = [];
              console.log("Distinct email_ids from annotation: ");
              // Loop only for logging. To be removed.
              for(var i in rows){
                  console.log(rows[i].email_id);
              }
              ...
              async.map(rows,
                function(row, callback){
                    return self._calculateAVGPrecision(row.email_id, db, callback);
                },
...
StatsEngine.prototype._calculateAVGPrecision = function(email_id, db, callback){
    const is_irrelevant = 0;
    db.query('select m.email_id, m.entity_title, a.rank, m.tf_idf from emailforensics.mv_tfidf m, emailforensics.annotation a where m.email_id =? and m.entity_title = a.entity_title and m.email_id = a.email_id order by tf_idf desc;', email_id, function(err, rows, fields) {
        if (!err){
