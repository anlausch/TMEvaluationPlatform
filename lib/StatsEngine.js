...
/**
 * Retrieves the number of documents from the database, which are annotated by a user
 * 
 * @returns {Integer} count
 * 
 */
StatsEngine.prototype.getNumberOfDocumentsAnnotated = function(fnCallback){
    var db = this.db;
...
/**
 * Retrieves the number of annotations from the database
 * 
 * @returns {Integer} count
 * 
 */
StatsEngine.prototype.getNumberOfAnnotations = function(fnCallback){
    var db = this.db;
...
/**
 * Retrieves the number of annotations in which the user did not select any entity
 * 
 * @returns {Integer} count
 * 
 */
StatsEngine.prototype.getNumberOfAnnotationsNoEntitySelected = function(fnCallback){
    var db = this.db;
...
StatsEngine.prototype.getTfidfPrecisionAtN = function(n, fnCallback){
    var db = this.db;
    db.query('SELECT tp/tpfp AS p FROM (SELECT COUNT(*) AS tp FROM (SELECT documentId, entityTitle, tfIdf FROM '
            +'(SELECT documentId, entityTitle, tfIdf, @doc_rank := IF(@current_doc = documentId, @doc_rank + 1, 1) AS doc_rank, '
            +'@current_doc := documentId FROM mv_tfidf ORDER BY documentId, tfIdf DESC) ranked WHERE doc_rank <= ?) AS a, entityselectionannotation b '
            +'WHERE a.documentId = b.documentid AND a.entitytitle = b.entitytitle AND rank > 0 AND (b.documentid, b.timestamp) NOT IN '
            +'(SELECT documentid, timestamp FROM entityselectionannotation GROUP BY rank, documentid, userusername, timestamp HAVING COUNT(*)=15)) AS tp, '
            +'(SELECT COUNT(*) AS tpfp FROM (SELECT documentId, entityTitle, tfIdf FROM (SELECT documentId, entityTitle, tfIdf, @doc_rank := IF(@current_doc = documentId, '
            +'@doc_rank + 1, 1) AS doc_rank, @current_doc := documentId FROM mv_tfidf ORDER BY documentId, tfIdf DESC) ranked WHERE doc_rank <= ?) AS a, '
            +'entityselectionannotation b WHERE a.documentId = b.documentid AND a.entitytitle = b.entitytitle AND (b.documentid, b.timestamp) NOT IN '
            +'(SELECT documentid, timestamp FROM entityselectionannotation GROUP BY rank, documentid, userusername, timestamp HAVING COUNT(*)=15)) AS tpfp', [n, n], function(err, rows, fields) {
        if (!err){
            if(rows){
                fnCallback(rows[0]);
            }
...
StatsEngine.prototype.getLLDAPrecision = function(fnCallback){
    var db = this.db;
    db.query('SELECT tp/tpfp AS p FROM (SELECT COUNT(*) AS tp FROM documenttopicdistribution a, entityselectionannotation b '
            +'WHERE a.documentid = b. documentid AND a.entitytitle = b.entitytitle AND rank > 0 AND (b.documentid, b.timestamp) '
            +'NOT IN (SELECT documentid, timestamp FROM entityselectionannotation GROUP BY rank, documentid, userusername, timestamp HAVING COUNT(*)=15)) AS tp, '
            +'(SELECT COUNT(*) AS tpfp FROM documenttopicdistribution a, entityselectionannotation b '
            +'WHERE a.documentid = b. documentid AND a.entitytitle = b.entitytitle AND (b.documentid, b.timestamp) '
            +'NOT IN (SELECT documentid, timestamp FROM entityselectionannotation GROUP BY rank, documentid, userusername, timestamp HAVING COUNT(*)=15)) AS tpfp', function(err, rows, fields) {
        if (!err){
            if(rows){
                fnCallback(rows[0]);
            }
 ...
 * 
 */
StatsEngine.prototype.calculateMAP = function(mode, onlyTop5, fnCallback){
    var db = this.db;
    var self = this;
          ...
          if(rows){
              ...
              async.map(
                    ...
                    function(row, callback){
                        return (mode === "tfidf" ? self._calculateAVGPrecisionTfIdf(row.documentid, row.timestamp, onlyTop5, db, callback) : self._calculateAVGPrecisionLLDA(row.documentid, row.timestamp, onlyTop5, db, callback));
                    },
 ...
 * 
 */
StatsEngine.prototype._calculateAVGPrecisionTfIdf = function(email_id, timestamp, onlyTop5, db, callback){
    const self = this;
    db.query('select a.documentid, a.entitytitle, a.rank, d.tfidf as ourrank from entityselectionannotation a left outer join mv_tfidf d on (d.documentid = a.documentid and d.entitytitle = a.entitytitle)  where a.documentid =? and a.timestamp=?  order by ourrank desc, rank asc;', [email_id, timestamp], function(err, rows, fields) {
        if (!err){
            ...
            var avgPrecision = self._getAVGPrecision(rows, onlyTop5);
 ...
 * 
 */
StatsEngine.prototype._calculateAVGPrecisionLLDA = function(email_id, timestamp, onlyTop5, db, callback){
    const self = this;
 ...
 * 
 */
StatsEngine.prototype._getAVGPrecision = function(rows, onlyTop5){
    ...
    // take only the top 5 tfidf values if requested, otherwise the top 10, as they belong to the doc
    rows = (onlyTop5 ? rows.slice(0,5) : rows.slice(0,10));
